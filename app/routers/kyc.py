...
async def upload_kyc_document(
    ...
    try:
        new_kyc = await KycService.upload_kyc_documents(
            db=db,
...
async def get_kyc_documents(request: Request, db: AsyncSession = Depends(get_db), current_user: db_models.User = Depends(au.get_current_user)):
    ip_address = request.client.host if request.client else "unknown"
    user_agent = request.headers.get("user-agent") if request else None
    logger.info(f"User {current_user.id} ({current_user.email}) retrieving all KYC documents from IP: {ip_address}")

    try:
        result = await db.execute(select(db_models.KYCDocument).where(db_models.KYCDocument.user_id == current_user.id))
        merchant_documents = result.scalars().all()
        ...
        return list(merchant_documents)
...
async def get_specific_document(id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: db_models.User = Depends(au.get_current_user)):
    ip_address = request.client.host if request.client else "unknown"
    user_agent = request.headers.get("user-agent") if request else None
    logger.info(f"User {current_user.id} ({current_user.email}) retrieving KYC document {id} from IP: {ip_address}")

    try:
        document_asked_for = await KycService.get_kyc_document_by_id(db=db, document_id=id, user=current_user)
...
async def delete_kyc_document(id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: db_models.User = Depends(au.get_current_user)):
    ip_address = request.client.host if request.client else "unknown"
    user_agent = request.headers.get("user-agent") if request else None
    ...
    try:
        result = await db.execute(
            ...
            .where(db_models.KYCDocument.id == id, db_models.KYCDocument.user_id == current_user.id)
        )
        ...
        await KycService.delete_kyc_document(db=db, document_id=id, user=current_user)
...
async def submit_kyc_for_review(
    ...
    try:
        kyc_verification = await KycService.submit_kyc_for_review(
            db=db,
            user=current_user,
...
async def get_kyc_status(
    ...
    try:
        status_data = await KycService.get_kyc_status(db=db, user=current_user)
...
async def get_required_actions(
    ...
    try:
        actions = await KycService.get_required_actions(db=db, user=current_user)
...
async def submit_identity_verification(
    ...
    try:
        identity = await KycService.create_or_update_identity(
            db=db,
            user_id=current_user.id,
            data=identity_data
        )
...
@router.get("/identity", response_model=kyc_schema.IdentityVerificationRes)
async def get_identity_verification(
    db: AsyncSession = Depends(get_db),
    current_user: db_models.User = Depends(au.get_current_user)
):
    identity = await KycService.get_identity(db=db, user_id=current_user.id)
    if not identity:
...
async def submit_business_verification_details(
    ...
    try:
        business = await KycService.create_or_update_business(
            db=db,
            user_id=current_user.id,
            data=business_data
        )
...
@router.get("/business", response_model=kyc_schema.BusinessVerificationRes)
async def get_business_verification(
    db: AsyncSession = Depends(get_db),
    current_user: db_models.User = Depends(au.get_current_user)
):
    business = await KycService.get_business(db=db, user_id=current_user.id)
    if not business:
